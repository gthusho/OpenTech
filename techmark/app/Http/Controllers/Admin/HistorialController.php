...
class HistorialController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(Auth::user()->can('allow-read')){
            if ($request)
            {
                ...
                $this->datos['historial'] = Clientes::with('cotizacionarticulo','ventaarticulo','cotizacionproducto','ventaproducto','produccion','visita')
    ...
    public function edit($id){
        if(Auth::user()->can('allow-read')){
                $this->datos['brand'] = Tool::brand('Historial',route('admin.cliente.index'),'Clientes');
