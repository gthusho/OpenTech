...
class ClienteController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(Auth::user()->can('allow-read')){
            if ($request)
            {
                $this->datos['brand'] = Tool::brand('Clientes',route('s.cliente.index'),'Clientes');
                $this->datos['clientes'] = Clientes::name($request->get('s'))
                    ->orderBy('id','desc')
                    ->paginate();
                return view('cpanel.sucursal.cliente.list')->with($this->datos);
            }
            else {
    ...
    public function create()
    {
        if(Auth::user()->can('allow-insert')){
            $this->datos['brand'] = Tool::brand('Crear Clientes',route('s.cliente.index'),'Clientes');
            return view('cpanel.sucursal.cliente.registro',$this->datos);
        }
    ...
    public function store(AddClienteRequest $request)
    {
        if(Auth::user()->can('allow-insert')){
            ...
            $cliente->save();
            return redirect()->route('s.cliente.index');
        }
    ...
    public function edit($id)
    {
        if(Auth::user()->can('allow-edit')){
            $this->datos['brand'] = Tool::brand('Editar cliente',route('s.cliente.index'),'Cliente');
            $this->datos['cliente'] = Clientes::find($id);
            return view('cpanel.sucursal.cliente.edit',$this->datos);
        }else{
